      ...
      (result: IsTenantAvailableOutput) => {
        switch (result.state) {
          ...
          case AppTenantAvailabilityState.InActive:
            this.message.add({ severity: 'warning', summary: this.l('TenantIsNotActive', this.tenancyName) });
            break;
          case AppTenantAvailabilityState.NotFound:
            this.message.add({ severity: 'warning', summary: this.l('ThereIsNoTenantDefinedWithName{0}', this.tenancyName) });
            break;
        }
      },
      () => {
