    ...
    public class LogInManager : AbpLogInManager<Tenant, Role, User>
    {
        ...
        public async Task<AbpLoginResult<Tenant, User>> LoginInternalOAuth2Async(string token, string tenancyName, bool shouldLockout)
        {
            ...
            try
            {
                ...
                using (UnitOfWorkManager.Current.SetTenantId(tenantId))
                {
                    ...
                    var user = UserManager.Users.FirstOrDefault(x => x.MezonUserId == userInfo.MezonId || x.EmailAddress == userInfo.Subject);
                    if (user == null)
                    {
        ...
        public async Task<AbpLoginResult<Tenant, User>> LoginAsyncInternalNoPass(string token, string secretCode, string tenancyName, bool shouldLockout)
        {
            ...
            try
            {
                ...
                // checking
                var clientAppId = await SettingManager.GetSettingValueAsync(AppSettingNames.GoogleClientAppId);//get googleClientAppId from setting
