    ...
    public class CVAutomationManager : BaseManager, ICVAutomationManager
    {
        ...
        private async Task<bool> SavingCV(CVScanResultFromFireBase CVInfor, KeyValuePair<string, Applicant> item)
        {
            try
            {
                ...
                {
                    TenantId = _session?.TenantId,
                    Name = item.Value.FullName.Length > CvAnnotationValue.Name ? item.Value.FullName[..CvAnnotationValue.Name] : item.Value.FullName,
                    Email = item.Value.Email,
                    Phone = item.Value.PhoneNumber.Length > CvAnnotationValue.Phone ? item.Value.PhoneNumber[..CvAnnotationValue.Phone] : item.Value.PhoneNumber,
                    Address = CVInfor.Address.Length > CvAnnotationValue.Address ? CVInfor.Address[..CvAnnotationValue.Address] : CVInfor.Address,
